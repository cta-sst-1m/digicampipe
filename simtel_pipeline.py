...
if __name__ == '__main__':
    ...
    parser.add_option('-d', "--baseline0", dest='baseline_beginning', help='N bins from the beginning of the waveform for baseline calculation', type=int, default=9)   # For searching of the best configuration
    parser.add_option('-e', "--baseline1", dest='baseline_end', help='N bins from the end of the waveform for baseline calculation', type=int, default=15)              #

    (options, args) = parser.parse_args()

    #data = '../data_test/ryzen_testprod_20180121/gamma_0deg_0deg_run3___cta-prod3-sst-dc-2100m--sst-dc.simtel.gz'
    ...
    time_integration_options = {'mask': None,
                                ...
                                'peak': None,
                                'window_start': 3,
                                'window_width': 15,      # sirkou okna se urci sirka integrovaneho signalu pro urcteni charge - melo by to odpovidat zhruba sirce ocekavaneho signalu
    ...
    #
    #'data'
    # Baseline is computed as a mean of 'n_bins0' first time samples, 'n_bins1' last time samples.
    # A key assumption in this method is that the shower in simulated data is somewhere in the middle of 50 samples..
    # Each pixel in each event has its own baseline
    #
    #'simtel'
    # Baseline is taken as simulated value event.mc.pedestal/50 that can be set up with the use of variable 'fadc_pedestal' in CTA-ULTRA6-SST-DC.cfg. 

    data_stream = simtel_baseline.fill_baseline_r0(data_stream, n_bins0=options.baseline_beginning, n_bins1=options.baseline_end, method='data')   #
    ...
    filename_eventsimage = 'events_image_'+ str(picture_threshold) + str(boundary_threshold)+ '_' + options.primary + '_ze' + str(options.zenit_angle).zfill(2) + '_az' + str(options.azimut).zfill(3) + '.txt'
    ...
    filename_baseline = 'baseline_' + options.primary + '_ze' + str(options.zenit_angle).zfill(2) + '_az' + str(options.azimut).zfill(3) + '_bas' + str(options.baseline_beginning).zfill(2) + str(options.baseline_end).zfill(2) + '.txt'
    #data_stream = simtel_baseline.save_mean_event_baseline(data_stream, directory + filename_baseline)
    ...
    for event in data_stream:
        ...
        print('')
        #print(geom)
        print(event.dl0.event_id, event.dl0.run_id, event.mc.energy, np.rad2deg(event.mc.alt), np.rad2deg(event.mc.az), event.mc.tel[telescope_id].time_slice)
        ...
        z1_plot=ax1.scatter(pix_x[dl1_camera.cleaning_mask] , pix_y[dl1_camera.cleaning_mask], c=dl1_camera.time_bin[1][dl1_camera.cleaning_mask])
        #z1_plot=ax1.scatter(pix_x , pix_y, c=dl1_camera.time_bin[1])
        plt.colorbar(z1_plot,ax=ax1)
        
        #fig = plt.figure(figsize=(10, 9))
        ax2.scatter(pix_x, pix_y, c='grey')
        z2_plot=ax2.scatter(pix_x[dl1_camera.cleaning_mask] , pix_y[dl1_camera.cleaning_mask], c=dl1_camera.pe_samples[dl1_camera.cleaning_mask])
        #z2_plot=ax2.scatter(pix_x , pix_y, c=dl1_camera.pe_samples)
        plt.colorbar(z2_plot,ax=ax2)
        
        '''
        fig = plt.figure(figsize=(10, 9))
        z2_plot=plt.scatter(pix_x , pix_y, c=event.r1.tel[telescope_id].adc_samples[:,16])
