...
class LidCCDImage(object):
    ...
    def __init__(
        ...
        if len(crop_pixels1) != len(crop_pixels2):
            raise AttributeError(
                'crop_pixels1 and crop_pixels2 lists '
        ...
        for crop_pixel1, crop_pixel2 in zip(crop_pixels1, crop_pixels2):
            image_cropped, crop_pixel1, crop_pixel2 = crop_image(
                filename, crop_pixel1, crop_pixel2)
            ratios = [
                self.image_shape[0] / image_cropped.shape[0],
                self.image_shape[1] / image_cropped.shape[1]
            ]
    ...
    def plot_image_solved(self, output_dir=None):
        ...
        it also shows the stars from the astrometry.net results.
        ...
        for sky_image, crop_pixel1, crop_pixel2 in zip(
                self.sky_images,
                self.crop_pixels1,
                self.crop_pixels2
        ):
    ...
    def plot_image_treated(self, output_dir=None):
        ...
        for sky_image, crop_pixel1, crop_pixel2 in zip(
            ...
            image_treated[
                crop_pixel1[1]:crop_pixel2[1],
                crop_pixel1[0]:crop_pixel2[0]
            ...
            if sky_image.wcs is not None:
                ...
                self.center_ra_dec = sky_image.wcs.wcs_pix2world(
                    self.center_px - crop_pixel1, 1)[0]
                ...
                for star_px, star_mag in zip(blue_stars_px, blue_stars_mag):
                    ...
                    circle = Circle(
                        (
                            star_px[0] + crop_pixel1[0],
                            star_px[1] + crop_pixel1[1]
                        ),
                        radius=radius,
                        fill=False,
                        color='b'
                ...
                circle = Circle(
                    (
                        crab_nebula_px[0] + crop_pixel1[0],
                        crab_nebula_px[1] + crop_pixel1[1]
                    ),
                    radius=20,
                    fill=False,
                    color='Y')
