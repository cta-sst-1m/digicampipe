...
looks for fits file.
For each distinct type of file found (based on the filenames), a corresponding 
...
def generate_container_class(path_file, file=sys.stdout):
    ...
    with fits.open(path_file) as hdul:
        ...
        if 'timestamp' not in lower_dict.keys():
            print('Warning in generate_container_class: no timestamp',
...
def generate_container_filler(path_file, file=sys.stdout):
    ...
    with fits.open(path_file) as hdul:
        ...
        for key in sorted(results):
            print('    event.slow_data.%s.%s = hdu.data["%s"][slow_event]'
...
def generate_header(file=sys.stdout):
    print('# WARNING ! this file has been generated by '
          'generate_slow_containers.py script.', file=file)
    print('# Do NOT edit this file manually as the changes would be '
          'lost when the script is re-run.', file=file)
