...
class RateThresholdAnalysis:
    ...
    def __call__(self, data_stream):
        rate = np.zeros(self.thresholds.shape)
        ...
        for i, event in enumerate(data_stream):
            for telescope_id in event.r0.tels_with_data:
                ...
                if i == 0:
                    n_clusters = trigger_input_7.shape[0]
                    cluster_rate = np.zeros(
                        (n_clusters, self.thresholds.shape[-1]))

                for j, threshold in enumerate(reversed(self.thresholds)):
                    comp = trigger_input_7 > threshold
                    if self.blinding:
                        if np.any(comp):
                            if self.by_cluster:
                                index = np.where(comp)[0]
                                cluster_rate[index, self.thresholds.shape[0] - j - 1] += 1
                                rate[self.thresholds.shape[0] - j - 1] += 1
                            else:
                                rate[0:self.thresholds.shape[0] - j] += 1
                                break
                    else:
                        comp = np.sum(comp, axis=0)
                        comp = comp > 0
                        n_triggers = np.sum(comp)
                        if n_triggers > trigger_input_7.shape[-1] - 1:
                            rate[0:self.thresholds.shape[0] - j] += n_triggers
                            break
                        rate[self.thresholds.shape[0] - j - 1] += n_triggers
            yield event
