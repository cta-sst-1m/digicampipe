...
class EventViewer():
    def __init__(
        ...
        self.cluster_matrix = np.zeros((len(self.camera.Clusters_7),
                                        len(self.camera.Clusters_7)))
    ...
    def next(self, event=None, step=1):
        ...
        self.telescope_id = event.r0.tels_with_data[0]
        ...
        try:
            ...
            zero_image = np.zeros(self.adc_samples.shape)
    ...
    def compute_trace(self):
        ...
        if self.readout_view_type in self.readout_view_types:
            ...
            if self.readout_view_type == 'raw':
                ...
                image = np.zeros(self.adc_samples.shape)
    ...
    def draw_on_camera(self, to_draw_on, event=None):
        ...
        if to_draw_on == 'hillas':
            self.hillas = not self.hillas

        if to_draw_on == 'mask':
            self.mask_pixels = not self.mask_pixels
