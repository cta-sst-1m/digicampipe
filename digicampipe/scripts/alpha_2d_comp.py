...
def entry():
    ...
    parser.add_option(
        ...
        default='../../sst-1m_data/20171030/output_crab_dark1_18_noaddrow_minphot80.npz')
    parser.add_option("-o", "--output", dest="output", help="output filename",
                      default="alpha_2d", type=str)
    parser.add_option("-b", "--bining", dest="bining",
                      help="size of bins in alpha histogram [degrees]",
                      default=4, type=float)
    parser.add_option("-s", "--steps", dest="steps",
                      help="number of steps, resolution of 2d alpha plot",
                      default=100, type=int)
    parser.add_option("-l", "--lower_cut", dest="lower_cut",
                      help="lower cut on length/width ratio", default=1.5,
                      type=float)
    parser.add_option("-u", "--upper_cut", dest="upper_cut",
                      help="upper cut on length/width ratio", default=3,
                      type=float)
    ...
    # application of cuts
    mask = (data['size'] > 0) & (data['length'] / data['width'] > lw_min) & (
        data['length'] / data['width'] < lw_max) & (data['border'] == 0)
