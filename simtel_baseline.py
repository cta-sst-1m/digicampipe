    ...
    for count, event in enumerate(event_stream):
        ...
        for telescope_id in event.r0.tels_with_data:
            ...
            if method == 'data':
                ...
                r0_camera.standard_deviation = 1.5*np.ones(n_pixels)    # should be set up manualy and it should be probably equal to value of 'fadc_noise' variable from simtel configuration file
...
def save_mean_event_baseline(event_stream, filename):
    ...
    for i, event in enumerate(event_stream):
        ...
        for telescope_id in event.r0.tels_with_data:
            ...
            baseline.append(np.mean(r0_camera.baseline[dl1_camera.cleaning_mask]))
            std.append(np.mean(r0_camera.standard_deviation[dl1_camera.cleaning_mask]))
