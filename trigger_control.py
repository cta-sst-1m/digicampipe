...
if __name__ == '__main__':
    ...
    opts_parser.add_option('-d', '--directory', dest='directory',
                           help='path',
                           default='/home/alispach/data/CRAB_01/')
    opts_parser.add_option('-f', '--file', dest='filename',
                           help='file basename',
                           default='CRAB_01_0_000.%03d.fits.fz')
    opts_parser.add_option("-s", "--file_start", dest="file_start",
                           help="file starting index",
                           default=3, type=int)
    opts_parser.add_option("-e", "--file_end", dest="file_end",
                           help="file starting index",
                           default=23, type=int)
    opts_parser.add_option("-c", "--config_path", dest="config_path",
                           help="config file path",
                           default="/home/alispach/ctasoft/CTS/config/",
                           type=str)
    ...
    file_list = [filename % number for number in range(
        options.file_start, options.file_end + 1)]
    ...
    cluster_7_histogram = histogram.Histogram(bin_center_min=0,
                                              bin_center_max=1785,
                                              bin_width=1,
                                              data_shape=(432, ))

    cluster_19_histogram = histogram.Histogram(bin_center_min=0,
                                               bin_center_max=4845,
                                               bin_width=1,
                                               data_shape=(432, ))
    ...
    if not display:
        ...
        pixel_histogram = histogram.Histogram(
            filename=directory + pixel_histogram_filename)
        patch_histogram = histogram.Histogram(
            filename=directory + patch_histogram_filename)
        cluster_7_histogram = histogram.Histogram(
            filename=directory + cluster_7_histogram_filename)
        cluster_19_histogram = histogram.Histogram(
            filename=directory + cluster_19_histogram_filename)
        ...
        for i in range(pixel_histogram.data.shape[0]):
            ...
            mean = np.average(pixel_histogram.bin_centers,
                              weights=pixel_histogram.data[i])
            std = np.average((pixel_histogram.bin_centers - mean)**2,
                             weights=pixel_histogram.data[i])
            std = np.sqrt(std)
            skewness = np.average(
...
: {:0.2f} []\n kurtosis : {:0.2f} []\n entries : {}'.format(
            ...
            axis.step(pixel_histogram.bin_centers[mask],
