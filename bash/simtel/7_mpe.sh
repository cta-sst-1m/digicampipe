...
then
    ...
    figure_path=$(printf $DIGICAM_FOLDER'figures/mpe_fit_window_%03d_shift_%03d_pixel_%04d.pdf' $DIGICAM_INTEGRAL_WIDTH $DIGICAM_INTEGRAL_SHIFT $PIXEL)
    info=$(printf '_window_%03d_shift_%03d' $DIGICAM_INTEGRAL_WIDTH $DIGICAM_INTEGRAL_SHIFT)
    histo=$DIGICAM_FOLDER'mpe_charge_histo'$info'.fits'
    digicam-mpe fit combined --output=$output --ac_levels=$(tolist "${DIGICAM_AC_LEVEL[@]}") --pixel=$PIXEL $histo
    digicam-mpe save_figure --output=$figure_path --calib=$output --ac_levels=$(tolist "${DIGICAM_AC_LEVEL[@]}") $histo
fi
